...
class LandmarkMap {
 public:
  ...
  /// @details A custom copy constructor is needed because the cached per-category
  /// kd-tree indices are stored behind unique pointers and therefore cannot be
  /// copied implicitly. The indices are rebuilt from the copied landmark data.
  ...
  /// @details A custom copy assignment operator is needed for the same reason as
  /// the copy constructor: copying the map requires rebuilding the cached kd-tree
  /// indices instead of copying the unique ownership wrappers directly.
  /// @param other Landmark map to copy from.
